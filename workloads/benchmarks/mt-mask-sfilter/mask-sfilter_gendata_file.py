...
def generate_kernel_data_file(
    filename: str,
    data_size: int = data_size
) -> None:

    def format_arr(ctype, name, sz, arr):
        lines = []
        lines.append(f"{ctype} __attribute__((aligned(16))) {name}[{sz}] = ")
        lines.append("{")
    ...
    with open(filename, "w") as f:
        ...
        f.write(f"#define data_t int\n")
        f.write(format_arr("int",   "input",       "DATA_SIZE*DATA_SIZE", X))
        f.write("\n\n")
        f.write(format_arr("short", "mask",        "DATA_SIZE*DATA_SIZE", mask))
        f.write("\n\n")
        f.write(format_arr("int",   "verify_data", "DATA_SIZE*DATA_SIZE", result))
        f.write("\n")

if __name__ == "__main__":
# Example usage:
    generate_kernel_data_file("s3.h")